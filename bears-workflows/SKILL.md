...
- Ask the user to confirm all deck slots before generating the protocol, then run `puda-machine-vision-validation` with the `puda-opentrons-vision-validation` adapter before physical execution.
...
- Before any physical OT-2 execution, run `puda-machine-vision-validation` with the `puda-opentrons-vision-validation` adapter to verify deck-slot occupation and labware against the planned protocol.
